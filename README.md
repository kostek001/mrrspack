# MRRSPack Rhea

Minecraft 1.21.4, Fabric, Vanilla+
Created for SMP

### Added:

- [MOD] (client) Sound Physics Remastered
- [MOD] (client) Smooth Scrolling
- [MOD] (client) Raised
- [MOD] (client) Customizable Player Models
- [MOD] (client) Customizable Player Models Simple Voice Chat Addon
- [MOD] (client) Item Scroller + MaLiLib
- [MOD] (client) Falling Leaves
- [MOD] (client) 3D Skin Layers
- [MOD] (client) Resource Pack Overrides

- [SP] BLS Shaders
- [SP] Complementary Shaders - Reimagined

- [RP] Default Dark Mode
- [RP] Detailed Animations
- [RP] Fresh Animations
- [RP] Fancy Crops
- [RP] Aust's Better Leaves
- [RP] No Worldborder

- [MOD] (both) Simple Voice Chat
- [MOD] (both) Unlock All Recipes
- [MOD] (both) No Enderman Grief
- [MOD] (both) Leaves Us In Peace
- [MOD] (both) Leaf Me Alone
- [MOD] (both) ScalableLux
- [MOD] (both) C2ME
- [MOD] (both) Carpet
- [MOD] (both) Carpet Extra
- [MOD] (both) Spark
- [MOD] (both) Chunky
- [MOD] (both) No Chat Reports
- [MOD] (both) Simple Resource Loader
- [MOD] (both) Distant Horizons (dev build)
- [MOD] (both) Inertia Anti Cheat

- [MOD] (server) Audioplayer
- [MOD] (server) Voice Chat Interaction
- [MOD] (server) Simple Voice Chat Radio
- [MOD] (server) Simple Voice Chat Enhanced Groups
- [MOD] (server) EasyAuth
- [MOD] (server) Fabric Tailor
- [MOD] (server) Ledger
- [MOD] (server) Luckperms
- [MOD] (server) Pl3xMap
- [MOD] (server) NoEnd
- [MOD] (server) Image2Map

### Credits:

- [Addictive](https://github.com/skywardmc/additive)
- [Vanilla Tweaks](https://vanillatweaks.net/) – datapacks
